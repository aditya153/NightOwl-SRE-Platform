      ...
      <div className="flex items-start justify-between gap-4 mb-5 flex-wrap">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-[10px] flex-wrap mb-[6px]" style={{ fontFamily: 'var(--font-head)', fontSize: '20px', fontWeight: 700, color: 'var(--color-text)' }}>
            {data.title}
            <span className="text-[10.5px] font-bold px-2 py-[3px] rounded tracking-[0.05em] uppercase" style={{ fontFamily: 'var(--font-mono)', background: sev.bg, color: sev.color, border: `1px solid ${sev.border}` }}>
              {data.severity}
            </span>
          </div>
        ...
        <div className="flex gap-2 shrink-0">
          ...
          <button onClick={() => acknowledge(data.id)} disabled={isResolved || isAcking} className="bg-transparent text-text py-[7px] px-[14px] rounded-[6px] text-[12.5px] font-medium cursor-pointer transition-all hover:bg-bg3 disabled:opacity-50 disabled:cursor-not-allowed" style={{ border: '1px solid var(--color-border2)', fontFamily: 'var(--font-body)' }}>
            {isAcking ? 'Acknowledging...' : 'Acknowledge'}
          </button>
          <button onClick={() => authorizeFix(data.id)} disabled={!isActionable || isFixing} className="bg-green border-none text-black py-[7px] px-[14px] rounded-[6px] text-[12.5px] font-bold cursor-pointer flex items-center gap-[6px] transition-all hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed" style={{ fontFamily: 'var(--font-body)' }}>
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: '300px 1fr' }}>
        <div className="flex flex-col gap-3">
          <div className="bg-bg2 border border-border rounded-[10px] overflow-hidden">
            <div className="text-[9.5px] tracking-[0.12em] uppercase text-text3 px-[14px] pt-3 pb-[10px] border-b border-border" style={{ fontFamily: 'var(--font-mono)' }}>
              Context
            </div>
            <div className="p-[14px]">
              <p className="text-[13px] text-text2 leading-[1.6]">{data.description}</p>
              <div className="mt-3">
                <div className="text-[9.5px] tracking-[0.12em] uppercase text-text3 pb-2" style={{ fontFamily: 'var(--font-mono)' }}>
                  Infrastructure
                </div>
          ...
          <div className="bg-bg2 rounded-[10px] p-3" style={{ border: '1px solid rgba(232,69,60,0.2)' }}>
            <div className="text-[9.5px] tracking-[0.12em] uppercase text-red mb-[6px]" style={{ fontFamily: 'var(--font-mono)' }}>
              Root Cause
            </div>
