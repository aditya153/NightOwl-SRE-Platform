...
export default function IncidentDetail({ incidentId }) {
  ...
  if (isLoading || !data) return (
    <div className="flex-1 overflow-y-auto p-12 text-on-surface flex items-center justify-center font-mono">
    ...
    <div className="flex-1 overflow-y-auto p-10 h-full fade-in">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <button className="px-4 py-2 border border-outline-variant/20 hover:bg-surface-container-high rounded-md text-sm font-bold text-on-surface transition-all cursor-pointer hover-lift">
            ACKNOWLEDGE
          </button>
            ...
            <div className="space-y-3">
              ...
              {data.infrastructure.map((item, idx) => (
                <div key={idx} className="flex justify-between items-center text-xs">
                  ...
                  <span className="font-mono text-on-surface bg-surface-container-high px-2 py-0.5 rounded">{item.value}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="col-span-12 xl:col-span-8">
          <div className="bg-surface-container-lowest h-full min-h-[500px] rounded-xl border border-outline-variant/10 shadow-lg flex flex-col hover-glow scale-in">
            <div className="p-4 border-b border-outline-variant/10 bg-surface-container-low/30 flex items-center justify-between">
              <h3 className="font-label text-xs uppercase tracking-[0.2em] text-on-surface flex items-center gap-2">
            ...
            <div className="flex-1 p-6 space-y-6 overflow-y-auto font-mono text-sm">
              {data.agentChats.map((chat, idx) => (
                <div key={idx} className="flex gap-4">
                  <div className={`w-8 h-8 shrink-0 rounded flex items-center justify-center ${chat.class.includes('bg-') ? chat.class : 'bg-surface-container-highest text-on-surface'}`}>
                    <span className="material-symbols-outlined text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>{chat.icon}</span>
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-bold text-on-surface text-xs">{chat.sender}</span>
              ...
              <div className="flex gap-4 opacity-50 animate-pulse">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-bold text-on-surface text-xs">Fixer Agent</span>
